...
NODE_LIST[0]="50.50.1.3 50.50.1.4"
...
run_server() {
	...
	kill_iperf $NAMESPACE $SERVER_IP $SSH_KEY

	print_info $MODE "Start iperf server at $SERVER_IP"
	iperf_server_tcp $NAMESPACE $SERVER_IP $SSH_KEY $SERVER_LOG
	sleep 1
}
...
run_client() {
	...
	iperf_client_tcp $NAMESPACE $CLIENT_IP $SSH_KEY $SERVER_IP $CLIENT_LOG $TEST_DURATION
}
...
$PWD/get-iperf-log.sh ovs $NUM_NODES
